...
public class ScraperTest {
	...
	private String testFileLocation = "/testpages/grants-gov-table.html";
	
	private URL grantsTableHtmlUrl;
	
	@Before
	public void setup(){
		grantsTableHtmlUrl = TestUtil.getFileAsURL(testFileLocation);
	}

	@Test
	public void scrapeUrlReturnsHtml() throws IOException {
		String pageText = new Scraper().url(grantsTableHtmlUrl).execute();
	...
	public void testAsText() throws IOException {
		String pageContent = new Scraper().url(grantsTableHtmlUrl).asText().execute();
	...
	public void extractTableFromPage() throws Exception {
		String pageText = new Scraper().url(grantsTableHtmlUrl).tag("<table>", 3).execute();
	...
	public void extractLinksFromTableOnPage() throws Exception {
		List<URL> urls = new Scraper().url(grantsTableHtmlUrl).tag("<table>", 3).getLinks();
